...
<?php
  ...
  foreach ($students as $idx => &$s) {
    ...
    // individually check if each photo exists
    // array_push all the valid photos to $students[$idx]['photos']
    // send the entire array to slideshow.php 
?>
     ...
     var studCSS = student['stud_css'];
      ...
      if (student['stud_future_plans'] != "") {
        ...
        for (var x = 0; x < scholarships.length; x++) {
          scholarshipsHtml += "<li style=\"" + studCSS + "\">" + scholarships[x] + "</li>";
        }
        for (var x = 0; x < futurePlans.length; x++) {
          futurePlansHtml += "<li style=\"" + studCSS + "\">" + futurePlans[x] + "</li>";
        }
